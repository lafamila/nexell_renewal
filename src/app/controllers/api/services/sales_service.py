...
""".format(
        params["s_ddt_man_start"],
        params["s_ddt_man_end"],
        "" if int(params["s_ddt_man_show"]) == 1 else " or a.ddt_man IS NULL",
    )
...
def get_sales_approval_datatable(params):
    query = """SELECT IFNULL(dlivy_de, '') AS dlivy_de
                    ...
                    AND deleted = 0
                    """.format(
        params["s_ddt_man_start"], params["s_ddt_man_end"]
    )
    data = []
    if "s_bcnc_sn" in params and params["s_bcnc_sn"]:
...
def insert_account(params):
    ...
    if "cnnc_sn" in params and "dlivy_de" in params and params["dlivy_de"] != "":
        row = g.curs.execute(
            "SELECT log_sn FROM stock_log WHERE delng_sn=%(cnnc_sn)s", params
        )
        if row:
            ...
            g.curs.executemany(
                "UPDATE stock_log SET ddt_man=%(dlivy_de)s WHERE log_sn=%(log_sn)s",
                log_sns,
            )
...
def update_account(params):
    ...
    query = """UPDATE account SET {} WHERE delng_sn=%(s_delng_sn)s""".format(
        ",".join(sub_query)
    )
    g.curs.execute(query, params)
    ...
    row = g.curs.execute(
        "SELECT delng_sn, sales_type, json_data FROM account_temp WHERE delng_sn=%(s_delng_sn)s",
        params,
    )
    if row:
        ...
        g.curs.execute(
            "SELECT ddt_man FROM account WHERE delng_sn=%(s_delng_sn)s", params
        )
        ...
        if account["ddt_man"] is not None:
            ...
            if data["sales_type"] == 0:
                g.curs.execute(
                    """INSERT INTO stock_log(stock_sn, stock_sttus, cntrct_sn, delng_sn, ddt_man, cnnc_sn) VALUES(%(stock_sn)s, 3, %(cntrct_sn)s, %(delng_sn)s, %(ddt_man)s, %(cnnc_sn)s)""",
                    json_data,
                )
            else:
                g.curs.execute(
            ...
            g.curs.execute(
                "DELETE FROM account_temp WHERE delng_sn=%(s_delng_sn)s", params
            )
...
def get_p_account_list(params):
    params["s_start_ddt_man"] = datetime.datetime.strptime(
        params["s_ddt_man"], "%Y-%m-%d"
    ).strftime("%Y-%m-01")
...
def get_expect_s_list(params):
    ...
    if "expect_de" in params:
        ...
        query += " FROM (SELECT * FROM account WHERE delng_se_code='S' AND delng_ty_code='12' AND expect_de BETWEEN '{0} 00:00:00' AND '{1} 23:59:59') s ".format(
            first_day, last_day
        )
...
def get_expect_t_list(params):
    ...
    if "expect_de" in params:
        ...
        query += " AND t.collct_de BETWEEN '{0} 00:00:00' AND '{1} 23:59:59' ".format(
            first_day, last_day
        )
    else:
...
def get_expect_r_list(params):
    ...
    if "expect_de" in params:
        ...
        query += " LEFT JOIN (SELECT * FROM taxbil WHERE delng_se_code LIKE 'S%%' AND collct_de BETWEEN '{0} 00:00:00' AND '{1} 23:59:59') t ".format(
            first_day, last_day
        )
    else:
    ...
    if params["msh_approval_type"] in ("S", "B"):
        ...
        if params["msh_approval_type"] == "S":
            g.curs.execute(
                ...
                (
                    params["mh_place"],
                    params["mh_count"],
                    params["mh_period"] + "~" + params["mh_period_end"],
                    cntrct_sn,
                ),
            )
        else:
            ...
            g.curs.execute(
                "SELECT log_sn, stock_sn, cnnc_sn FROM stock_log WHERE delng_sn IN ({})".format(
                    ",".join(["%s"] * len(delng_sns))
                ),
                delng_sns,
            )
            ...
            for stock in stocks:
                if stock["cnnc_sn"] is None:
                    g.curs.execute(
                        "DELETE FROM stock WHERE stock_sn=%s", stock["stock_sn"]
                    )
                g.curs.execute("DELETE FROM stock_log WHERE log_sn=%s", stock["log_sn"])

            g.curs.execute(
                "UPDATE contract SET mh_place=%s, mh_count=%s, mh_period=%s WHERE cntrct_sn=%s",
                (
                    params["mh_place"],
                    params["mh_count"],
                    params["mh_period"] + "~" + params["mh_period_end"],
        ...
        for key in params:
            if key.endswith("[]"):
                if key == "pamount[]":
                    data["dlamt"] = [
                        int(d.replace(",", "")) if d.replace(",", "") != "" else 0
                        for d in params[key]
                    ]
                elif key == "dlivy_amt[]":
                    data["dlivy_amt"] = [
                    ...
                    data["dscnt_rt"] = [
                        float(d.replace("%", "")) if d.replace("%", "") != "" else 0.0
                        for d in params[key]
                    ]
        ...
        for i in range(len(input_data)):
            ...
            if (
                ...
                and row["dlivy_amt"] == 0
                and row["dlnt"] == 0
                and row["add_dscnt_rt"] == 0.0
                and row["dscnt_rt"] == 0.0
                and row["model_no"].strip() == ""
            ):
                deletes.append(i)
        ...
        if params["msh_approval_type"] == "B":
            g.curs.execute(
                "DELETE FROM cost WHERE cntrct_sn=%(cntrct_sn)s AND cntrct_execut_code='E' AND ct_se_code IN ('61', '62', '64', '65')",
                params,
            )
        for row in input_data:
            ...
            row["add_dscnt_rt"] = (
                (add_dc * 100.0 / row["b61"]) if row["b61"] != 0 else 0.0
            )
        ...
        cntrct_sn = params["cntrct_sn"]
        g.curs.execute(
            "UPDATE contract SET mh_approval_step=2 WHERE cntrct_sn=%s", (cntrct_sn,)
        )
...
def insert_ms_equip(params):
    if params["msh_approval_type"] in ("S", "B"):
        ...
        if params["msh_approval_type"] == "S":
            ...
            g.curs.execute(
                "SELECT delng_sn FROM account WHERE cntrct_sn=%(cntrct_sn)s AND (delng_se_code='P' and delng_ty_code IN ('61', '62')) OR cnnc_sn IN (SELECT delng_sn FROM account WHERE delng_se_code='P' and delng_ty_code IN ('61', '62') AND cntrct_sn=%(cntrct_sn)s)",
                params,
            )
            accounts = g.curs.fetchall()
            delng_sns = [acc["delng_sn"] for acc in accounts]
        ...
        for i in range(len(input_data)):
            ...
            if row["dlamt"] == 0 and row["dlnt"] == 0 and row["model_no"].strip() == "":
                deletes.append(i)
            elif row["bcnc_sn"] == "" or int(row["bcnc_sn"]) not in (74, 79, 100):
                deletes.append(i)
        print(deletes)
        for i in deletes[::-1]:
            del input_data[i]
...
def insert_equipment_sub(params):
    ...
    for order_de, prdlst_se_code, pamt, samt, bcnc_sn in zip(
        order_des, prdlst_se_codes, damts, damt2s, bcnc_sns
    ):
        ...
        g.curs.execute(
            "INSERT INTO equipment({}) VALUES ({})".format(
...
def insert_equipment_other_BD(params):
    ...
    for (
        ...
        params["bcnc_sn[]"],
        params["cntrct_amount[]"],
        params["dlivy_amt[]"],
        params["dlnt[]"],
        params["amount[]"],
        params["model_no[]"],
        params["pamount[]"],
        params["prdlst_se_code[]"],
    ):
        ...
        row["regist_dtm"] = datetime.datetime.now(timezone("Asia/Seoul"))
        row["register_id"] = (
            session["member"]["member_id"] if "member" in session else "nexelll"
        )
        row["delng_se_code"] = "P"
        row["ddt_man"] = None
        row["bcnc_sn"] = 100
        ...
        keys = list(row.keys())
        g.curs.execute(
            "INSERT INTO account({0}) VALUES ({1})".format(
                ",".join(keys), ",".join(["%({})s".format(key) for key in keys])
            ),
            row,
        )
        delng_sn = g.curs.lastrowid
...
def insert_equipment_other_sub(params):
    ...
    damts = params["damt[]"]
    bcnc_sns = params["bcnc_sn[]"]
    for order_de, model_no, prdlst_se_code, damt, dlnt, bcnc_sn in zip(
        order_des, model_nos, prdlst_se_codes, damts, dlnts, bcnc_sns
    ):
        if order_de == "" or prdlst_se_code == "":
            continue
        data["order_de"] = order_de
        ...
        data["pamt"] = (
            int(damt.replace(",", "")) * int(dlnt.replace(",", ""))
            if damt != ""
            else None
        )
        data["samt"] = data["pamt"]
...
def update_equipment_establish(params):
    ...
    for idx, c in company.items():
        ...
        if int(idx.replace("company", "")) - 1 < len(outsrcs):
            ...
            for item_type, item_nm, item_dlnt, item_damt, item_total in zip(
                c["item_type[]"],
                c["item_name[]"],
                c["item_dlnt[]"],
                c["item_damt[]"],
                ...
                if item_nm != "":
                    ...
                    if int(item_type) == 3:
                        row = g.curs.execute(
                            "SELECT outsrc_sn FROM outsrc WHERE cntrct_sn=%(cntrct_sn)s AND outsrc_fo_sn=146",
                            c,
                        )
                        if row == 0:
...
def insert_equipment_new(params):
    ...
    for key in params:
        if (
            key.endswith("[]")
            and not key.startswith("company")
            and not key.endswith("other[]")
        ):
        ...
        for i, v in enumerate(value):
            if not key.endswith("other[]"):
                ...
                if k in ("cnt_dlnt", "dlamt"):
                    equipments[i][k] = (
                        0 if v.replace(",", "") == "" else int(v.replace(",", ""))
                    )
                elif k == "samount":
                    equipments[i][k] = (
                        None if v.replace(",", "") == "" else int(v.replace(",", ""))
                    )
                else:
                    equipments[i][k] = v
    ...
    equipments = [
        ...
        if eq["bcnc_sn"] != "" and eq["dlamt"] != 0 and eq["model_no"] != ""
    ]
    equipments_other = [
        eq
        for eq in equipments_other
        if eq["bcnc_sn"] != "" and eq["dlamt"] != 0 and eq["model_no"] != ""
    ]
        ...
        for eq in equipments_update:
            ...
            if row:
                ...
                if before["samt"] is None:
                    before["samt"] = 0
                before["samt"] = int(
                    before["samt"] * int(eq["cnt_dlnt"]) / before["cnt_dlnt"]
                )
                before["cnt_dlnt"] = int(eq["cnt_dlnt"])
                keys = list(before.keys())
                query = """INSERT INTO expect_equipment({0}) VALUES ({1})""".format(
...
def insert_RD_equipment(params):
    cost_data = []
    prjct = get_project_by_cntrct_nm(params["cntrct_sn"])
    for key in params:
        if key.endswith("[]"):
    ...
    for idx, c in company.items():
        ...
        if s != 0:
            ...
            if row:
                cntrwk_ct_sn = g.curs.fetchone()["cntrwk_ct_sn"]
                g.curs.execute(
                    "DELETE FROM cost WHERE cntrwk_ct_sn=%s", (cntrwk_ct_sn,)
                )
            query = """INSERT INTO cost({}) VALUES ({})""".format(
                ",".join(sub_query), ",".join(params_query)
            )
...
def insert_RD_equipment_extra(params):
    cost_data = []
    prjct = get_project_by_cntrct_nm(params["cntrct_sn"])
    ...
    for idx, c in company.items():
        ...
        c["register_id"] = session["member"]["member_id"]
        c["outsrc_type"] = 1
        query = """INSERT INTO outsrc(cntrct_sn, prjct_sn, outsrc_fo_sn, cntrct_de, pymnt_mth, rspnber_nm, rspnber_telno, charger_nm, charger_telno, regist_dtm, register_id, outsrc_type)
                    VALUES(%(cntrct_sn)s, %(prjct_sn)s, %(name)s, %(date)s, %(pymnt_mth)s, %(owner)s, %(owner_telno)s, %(dam)s, %(dam_telno)s, NOW(), %(register_id)s, %(outsrc_type)s)"""
        g.curs.execute(query, c)
...
def insert_RD_equipment_other(params):
    data = OrderedDict()
    data["cntrct_sn"] = params["cntrct_sn"]
    ...
    damts = params["dlamt[]"]
    bcnc_sns = params["bcnc_sn[]"]
    samounts = params["now_samount[]"]
    for model_no, prdlst_se_code, damt, samt, dlnt, bcnc_sn in zip(
        model_nos, prdlst_se_codes, damts, samounts, dlnts, bcnc_sns
    ):
        data["order_de"] = datetime.datetime.now(timezone("Asia/Seoul")).strftime(
            "%Y-%m-%d"
        )
        data["prdlst_se_code"] = prdlst_se_code
        data["model_no"] = "자재"
...
def insert_equipment_samsung(params):
    ...
    if "option_bigo" in params and params["option_bigo"].strip() != "":
        ...
        partclr_matter = "{}\n\n{} {}\n{}".format(
            before,
            datetime.datetime.now(timezone("Asia/Seoul")).strftime("%Y-%m-%d"),
            "삼성장비" if "_type" not in params else "타사장비",
            params["option_bigo"],
        )
...
def update_account_expect_de(params):
    g.curs.execute(
        "UPDATE account SET expect_de=%(expect_de)s WHERE cntrct_sn=%(cntrct_sn)s AND bcnc_sn=%(bcnc_sn)s AND expect_de=%(before)s",
        params,
    )
...
def insert_general_sales_BD(params):
    ...
    now_bgn = datetime.datetime.strptime(
        datetime.datetime.now(timezone("Asia/Seoul")).strftime("%Y-%m-01"), "%Y-%m-%d"
    )
    ...
    for (
        ...
        data["dlivy_amt"] = (
            int(dlivy_amt.replace(",", "")) if dlivy_amt.replace(",", "") != "" else 0
        )
        data["dlnt"] = int(dlnt.replace(",", "")) if dlnt.replace(",", "") != "" else 0
        data["dscnt_rt"] = (
            float(dc.replace("%", "")) if dc.replace("%", "") != "" else 0.0
        )
        if stock_sn != "":
            data["ddt_man"] = None
            data["dscnt_rt"] = None
        ...
        if stock_sn != "":
            ...
            g.curs.execute(
                "INSERT INTO account_temp(delng_sn, sales_type, json_data) VALUES (%s, 1, %s)",
                (cnnc_sn, json.dumps(json_data)),
            )
    ...
    if total_samount > 0:
        g.curs.execute(
            "INSERT INTO contract_table(cntrct_de, cntrct_sn, cntrct_amount) VALUES (%(cntrct_de)s, %(cntrct_sn)s, %(total_samount)s)",
            {
                "cntrct_de": cntrct_de,
                "cntrct_sn": general_cntrct_sn,
                "total_samount": total_samount,
            },
        )
...
def insert_general_sales_NR(params):
    ...
    for (
        ...
        data["delng_se_code"] = "P"
        data["dlamt"] = (
            int(dlamt.replace(",", "")) if dlamt.replace(",", "") != "" else 0
        )
        data["dlnt"] = int(dlnt.replace(",", "")) if dlnt.replace(",", "") != "" else 0
        data["model_no"] = model_no
        data["prdlst_se_code"] = prdlst_se_code
        ...
        if stock_sn != "":
            ...
            g.curs.execute(
                "INSERT INTO account_temp(delng_sn, sales_type, json_data) VALUES (%s, 0, %s)",
                (cnnc_sn, json.dumps(json_data)),
            )
            # g.curs.execute("""INSERT INTO stock_log(stock_sn, stock_sttus, cntrct_sn, delng_sn, ddt_man, cnnc_sn) VALUES(%s, 3, %s, %s, %s, %s)""", (stock_sn, data['cntrct_sn'], cnnc_sn, data['ddt_man'], log_sn))
        ...
        if samount > 0:
            if general_cntrct_sn is None:
                g.curs.execute(
                    "SELECT m.dept_code, c.bcnc_sn FROM contract c LEFT JOIN member m ON c.bsn_chrg_sn=m.mber_sn WHERE c.cntrct_sn=%s",
                    params["cntrct_sn"],
                )
                contract = g.curs.fetchone()
                dept_code = contract["dept_code"]
                bcnc_sn = contract["bcnc_sn"]
                dept_nm = {"TS1": "공조1", "TS2": "공조2", "BI": "빌트인"}.get(
                    dept_code, "영업"
                )
                now_bgn = datetime.datetime.strptime(
                    datetime.datetime.now(timezone("Asia/Seoul")).strftime("%Y-%m-01"),
                    "%Y-%m-%d",
                )
                cntrct_nm_format = now_bgn.strftime("%y년%m월 일반판매")
                cntrct_nm = "{}({})".format(cntrct_nm_format, dept_nm)
                g.curs.execute(
                    "SELECT cntrct_sn FROM contract WHERE cntrct_nm=%s", cntrct_nm
                )
                print(cntrct_nm)
                general_cntrct_sn = g.curs.fetchone()["cntrct_sn"]
...
def insert_general_sales_NR_append(params):
    general_cntrct_sn = 13053
    now = datetime.datetime.now(timezone("Asia/Seoul")).strftime("%Y-%m-%d")
    total_samount = 0
    cost_data = []
    cost_date = datetime.datetime.now(timezone("Asia/Seoul"))
    for (
        sale_type,
        stock_sn,
        bcnc_sn,
        ...
        cntrct_execut_code = "C"  # 'E'
        ct_se_code = 1 if int(bcnc_sn) in (74, 79) else 2
        column = "puchas_amount" if cntrct_execut_code == "E" else "salamt"
        cost_data.append(
            {
                "cntrct_sn": params["cntrct_sn"],
                "prjct_sn": data["prjct_sn"],
                "cntrct_execut_code": "E",
                "ct_se_code": ct_se_code,
                "model_no": model_no,
                "qy": data["dlnt"],
...
def get_equipment(params):
    row = g.curs.execute(
        "SELECT eq_sn FROM equipment WHERE cntrct_sn=%(cntrct_sn)s AND model_no=%(model_no)s AND pamt*(dlnt-before_dlnt)=%(amount)s AND deleted = 0",
        params,
    )
    result = g.curs.fetchall()
    return result
