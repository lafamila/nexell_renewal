...
def get_taxbil_datatable(params):
    query = """SELECT t.ctmmny_sn
				...
				WHERE 1=1
				AND t.ctmmny_sn = '1'
				AND t.pblicte_de BETWEEN '{0} 00:00:00' AND '{1} 23:59:59' """.format(
        params["s_pblicte_de_start"], params["s_pblicte_de_end"]
    )
...
def insert_taxbil(params):
    ...
    query = """INSERT INTO taxbil({}) VALUES ({})""".format(
...
def update_taxbil(params):
    ...
    query = """UPDATE taxbil SET {} WHERE taxbil_sn=%(s_taxbil_sn)s""".format(
        ",".join(sub_query)
    )
    g.curs.execute(query, params)
