    ...
    query = """SELECT c.progrs_sttus_code
				...
				, (
                    SELECT IFNULL(
                        SUM(
                            IF( c.prjct_ty_code <> 'BF',
                                IF ( c.prjct_ty_code = 'NR' , 
                                    IF( c.progrs_sttus_code <> 'B', 
                                        IF( cst.cntrct_execut_code = 'B' or cst.extra_sn < ct.max_sn,
                                            0, 
                                            IFNULL(cst.salamt,0)*cst.qy), 
                                        0),
                                    IF( c.progrs_sttus_code <> 'B', 
                                        IF( cst.cntrct_execut_code = 'B',
                                            0, 
                                            IFNULL(cst.salamt,0)*cst.qy), 
                                        0)
                                ),
                                IF(cst.cntrct_execut_code = 'C', IFNULL(cst.QY, 0)*IFNULL(cst.puchas_amount,0)*0.01*(100.0-IFNULL(cst.dscnt_rt, 0))*IFNULL(cst.fee_rt, 0)*0.01, 0))),0) FROM cost cst LEFT JOIN (SELECT max(extra_sn) AS max_sn, cntrct_sn FROM cost GROUP BY cntrct_sn) ct ON cst.cntrct_sn=ct.cntrct_sn WHERE cst.cntrct_sn = c.cntrct_sn AND cst.cntrct_execut_code <> 'B') AS cntrct_amount
				, m.dept_code
				, m.mber_nm
				, (SELECT code_nm FROM code WHERE parnts_code='DEPT_CODE' AND code=m.dept_code) AS dept_nm
...
def get_logitech_report(params):
    ...
    , (SELECT SUM(IFNULL(s.dlnt, 0)*IFNULL(s.dlamt, 0)) FROM (SELECT * FROM account WHERE delng_se_code='P') p LEFT JOIN (SELECT * FROM account WHERE delng_se_code='S') s ON s.cnnc_sn=p.delng_sn WHERE p.delng_ty_code IN ('3', '4') AND p.cntrct_sn=c.cntrct_sn AND p.bcnc_sn=3 AND DATE_FORMAT(s.ddt_man, '%%Y-%%m')='{0}') AS logi_now
    , (SELECT SUM(IFNULL(s.dlnt, 0)*IFNULL(s.dlamt, 0)) FROM (SELECT * FROM account WHERE delng_se_code='P') p LEFT JOIN (SELECT * FROM account WHERE delng_se_code='S') s ON s.cnnc_sn=p.delng_sn WHERE p.delng_ty_code IN ('3', '4') AND p.cntrct_sn=c.cntrct_sn AND p.bcnc_sn=3 AND DATE_FORMAT(s.ddt_man, '%%Y-%%m') < '{0}') AS logi_before
    , (SELECT SUM(IFNULL(p.dlnt, 0)*IFNULL(p.dlamt, 0)) FROM (SELECT * FROM account WHERE delng_se_code='P') p WHERE p.delng_ty_code IN ('3', '4') AND p.cntrct_sn=c.cntrct_sn AND p.delng_ty_code='4' AND DATE_FORMAT(p.ddt_man, '%%Y-%%m') = '{0}') AS other_now
    , (SELECT SUM(IFNULL(p.dlnt, 0)*IFNULL(p.dlamt, 0)) FROM (SELECT * FROM account WHERE delng_se_code='P') p WHERE p.delng_ty_code IN ('3', '4') AND p.cntrct_sn=c.cntrct_sn AND p.delng_ty_code='4' AND DATE_FORMAT(p.ddt_man, '%%Y-%%m') < '{0}') AS other_before
