...
def history_login(view_title, view_action, member_sn, member_id):
    row = g.curs.execute(
        """INSERT INTO history(client_sn, member_sn, view_title, view_path, view_action, regist_dtm, register_id) VALUES (1, %s, %s, '/', %s, NOW(), %s)""",
        (member_sn, view_title, view_action, member_id),
    )
    return {"status": True}
...
def update_image(params):
    if int(params["purpose"]) == 0:
        query = (
            """UPDATE member SET sign_file_sn=%(file_sn)s WHERE mber_sn=%(mber_sn)s"""
        )
    else:
        query = (
            """UPDATE member SET prof_file_sn=%(file_sn)s WHERE mber_sn=%(mber_sn)s"""
        )
...
def update_member(params):
    ...
    sub_query = [
        (
            "{0}=%({0})s".format(key)
            if key != "mber_password"
            else "{0}=password(%({0})s)".format(key)
        )
        for key in data
    ]
    query = (
        """UPDATE member SET {}, UPDATE_DTM=NOW() WHERE mber_sn=%(mber_sn)s""".format(
            ",".join(sub_query)
        )
    )
    g.curs.execute(query, params)
...
def insert_member(params):
    ...
    query = """INSERT INTO member({}) VALUES ({})""".format(
...
def get_team_leader(dept_code):
    row = g.curs.execute(
        "SELECT mber_sn FROM member WHERE dept_code=%s AND mber_sttus_code='H' AND RSPOFC_CODE=150",
        dept_code,
    )
    if not row:
        return 63
...
def get_member_info(member_sn):
    ...
    FROM member m LEFT JOIN ctmmny c ON m.ctmmny_sn = c.ctmmny_sn LEFT OUTER JOIN files f ON f.f_sn=m.prof_file_sn WHERE m.mber_sn=%s""",
        (member_sn,),
    )
    member = g.curs.fetchone()
    ...
...
def update_member_password(member_sn, old_pwd, new_pwd):
    row = g.curs.execute(
        "SELECT * FROM member WHERE mber_sn=%s AND mber_password=PASSWORD(%s) AND mber_sttus_code='H'",
        (member_sn, old_pwd),
    )
    member = g.curs.fetchone()
    if member:
        g.curs.execute(
            "UPDATE member SET mber_password=PASSWORD(%s) WHERE mber_sn=%s",
            (new_pwd, member_sn),
        )
        return {
            "status": True,
            "message": "성공적으로 변경되었습니다. 재로그인이 필요합니다.",
        }
    else:
...
def member_check(member_id, member_pw):
    row = g.curs.execute(
        "SELECT mber_sn, mber_qr, mber_otp FROM member WHERE mber_id=%s AND mber_password=PASSWORD(%s) AND mber_sttus_code='H'",
        (member_id, member_pw),
    )
    member = g.curs.fetchone()
    ...
    if member:
        ...
        if secret_key == "" or secret_key is None:
            ...
            g.curs.execute(
                "UPDATE member SET mber_otp=%s WHERE mber_sn=%s",
                (secret_key, member_sn),
            )

        params = urllib.parse.urlencode(
            {
                "data": "otpauth://totp/nexellsystem?secret={}&issuer=lafamila".format(
                    secret_key
                )
            }
        )
        qrCodeUrl = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&" + params

        return {
            "status": True,
            "qrCodeUrl": urllib.parse.unquote(qrCodeUrl),
            "secret": secret_key,
            "mber_qr": member_qr,
            "mber_sn": member_sn,
        }
...
def qrcode_check(member_sn, member_qr):
    row = g.curs.execute(
        "SELECT mber_sn, mber_qr, mber_otp FROM member WHERE mber_sn=%s", (member_sn,)
    )
    member = g.curs.fetchone()
    secret_key = member["mber_otp"]
...
def get_member_todo(params):
    ...
    query = """				SELECT m.mber_nm
				...
				AND m.check_todo = 1
				{0}
				ORDER BY code_ordr, m.ofcps_code, m.rspofc_code, m.enter_de, ordr""".format(
        sub_query
    )
    g.curs.execute(query, params)
...
def insert_vacation(params):
    ...
    for d in range((ed_de - st_de).days + 1):
        ...
        g.curs.execute(
            "INSERT INTO vacation({}) VALUES ({})".format(
