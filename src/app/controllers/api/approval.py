...
def ajax_insert_approval():
    ...
    if int(params["approval_ty_code"]) in (1, 39, 40, 81):
        if (
            ...
            params["data"]["cntrct_no"] = prj.get_contract_no(
                {"today": datetime.today().strftime("%Y-%m-%d")},
                mber_sn=params["data"]["bsn_chrg_sn"],
            )
    ...
    if member["dept_code"] == "ST":
        ...
        if (
            required_member_sn not in coop_list
            and required_member_sn not in approval_list
        ):
            required_member = mber.get_member(required_member_sn)
            return make_response(
                f"해당 품의는 필수 협조자[{required_member['mber_nm']}]가 지정되어야 합니다.",
                501,
            )

        # 사장 91, 부회장 93, 회장 4
        required_member_sns = [91, 93, 4]
        for required_member_sn in required_member_sns[:-1]:
            if int(required_member_sn) not in approval_list:
                required_member = mber.get_member(required_member_sn)
                return make_response(
                    f"해당 품의는 필수 결재자[{required_member['mber_nm']}]가 포함되어야 합니다.",
                    501,
                )
        if int(required_member_sns[-1]) != approval_list[-1]:
            required_member = mber.get_member(int(required_member_sns[-1]))
            return make_response(
                f"해당 품의의 최상위 결재자[{required_member['mber_nm']}]가 일치하지 않습니다.",
                501,
            )
        ...
        ## 필수 협조자 체크
        # 지출결의서
        if approval_ty_code in (75, 76, 77):
            amount = (
                0
                if params["data"].get("amount", "") == ""
                else int(params["data"].get("amount", "").replace(",", ""))
            )
            if amount >= 50000:
                required_member_sns = [] + (
                    [63]
        ...
        # 출장보고서
        elif approval_ty_code in (20, 53, 54):
            required_member_sns = [59, 19] + (
                [63]
        ...
        for required_member_sn in required_member_sns:
            if (
                required_member_sn not in coop_list
                and required_member_sn not in approval_list
            ):
        ...
        ## 필수결재자 / 최상위 결재자 체크
        required_member_sns = list()
        required_member_sns.append(mber.get_team_leader(member["dept_code"]))
        # 지출결의서는 금액에 따라 다름
        if approval_ty_code in (75, 76, 77):
            amount = (
                0
        ...
        # 휴가신청서
        if (
            approval_detail["conditions"] != 0 and approval_ty_code in (56, 57, 22)
        ) or approval_detail["auth_type"] == 2:
            if approval_ty_code in (56, 57, 22):
                member = mber.get_member(approval_list[0])
                if (
                    int(200 if member["rspofc_code"] == "" else member["rspofc_code"])
                    != 200
                ):
                    required_member_sns.append(4)
...
def get_approval():
    try:
        ...
        result["html"] = apvl.get_approval_template(
            result["approval"]["template_url"], init=init
        )
        ...
        if isFired:
            ...
            if (
                last_member is not None
                and last_member[0] == session["member"]["member_sn"]
                and next_member is not None
            ):
                isCancelable = True

        result["approval"]["approval_data"] = json.loads(
            result["approval"]["approval_data"]
        )
...
def update_approval():
    try:
        ...
        return jsonify(
            {"status": True, "isEnd": False, "message": "성공적으로 처리되었습니다."}
        )
    except Exception as e:
        print(e)
        return make_response(str(e), 500)
