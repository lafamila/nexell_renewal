...
def ajax_insert_sales():
    try:
        ...
        if (
            ...
            and bcnc_sn == "74"
            and delng_ty_code == "1"
            and "invn_sttus_code" in params
            and params["invn_sttus_code"]
            and ("cntrct_sn" not in params or params["cntrct_sn"] == "")
            and int(params["dlnt"]) > 0
        ):
            # print(params)
            ...
            for _ in range(int(params["dlnt"])):
                ...
                st.insert_log(
                    item_sn, 1, params["invn_sttus_code"], delng_sn, params["ddt_man"]
                )
        ...
        # 창고->전시
            ...
            delng_se_code == "P"
            and bcnc_sn == "79"
            and delng_ty_code in ("62", "61")
            and "invn_sttus_code" in params
            and params["invn_sttus_code"]
            and "cntrct_sn" in params
            and params["cntrct_sn"]
            and int(params["dlnt"]) > 0
        ):
            ...
            if len(item_sns) > 0:
                for item_sn in item_sns:
                    st.insert_log(
                        item_sn, 2, params["cntrct_sn"], delng_sn, params["ddt_man"]
                    )
        ...
        # 창고->판매
            ...
            if len(item_sns) > 0:
                for item_sn in item_sns:
                    st.insert_log(
                        item_sn, 3, params["cntrct_sn"], delng_sn, params["ddt_man"]
                    )
            # 바로 구매->판매
            else:
                # for _ in range(int(params['dlnt'])):
        ...
        # 구매->현장 전시???
            ...
            delng_se_code == "P"
            and bcnc_sn in ("74", "100")
            and delng_ty_code == "61"
            and "invn_sttus_code" in params
            and params["invn_sttus_code"] == "0"
            and "cntrct_sn" in params
            and params["cntrct_sn"]
            and int(params["dlnt"]) > 0
        ):
...
def ajax_get_sales_expect_report():
    try:
        ...
        if "expect_de" in params:
            ...
            for i in range(3):
                params["expect_de"] = (
                    datetime.strptime(s_expect_de, "%Y-%m-%d")
                ...
                for t in t_result:
                    ...
                    if key not in t_dict:
                        ...
                        t_dict[key]["taxbil_sn"] = ",".join(
                            list(
                                set(t_dict[key]["taxbil_sn"].split(","))
                                | set(t["taxbil_sn"].split(","))
                            )
                        )
                ...
                for key in set(s_dict.keys()) | set(t_dict.keys()):
                    ...
                    if key in s_dict and key in t_dict:
                        ...
                        price_3 = t["price_total"] - sum(
                            [s["price_1"] + s["price_2"] for s in s_rows]
                        )
                        ...
                        for i in range(rows):
                            ...
                            if i < len(s_rows) and i < len(r_rows):
                                data = make_row(
                                    ...
                                    s_rows[i]["price_2"],
                                    price_3,
                                    s_rows[i]["price_1"] + s_rows[i]["price_2"] + p3,
                                    t["price_total"],
                                    t["delng_se_nm"],
                                    expect_de,
                                    r_rows[i]["rcppay_de"],
                        ...
                        if int(taxbil_sn) in r_dict:
                            ...
                            for r in r_rows:
                                row.append(
                                    {
                                        "bcnc_nm": t["bcnc_nm"],
                                        "cntrct_nm": t["cntrct_nm"],
                                        "price": t["price_total"],
                                        "expect_de": t["expect_de"],
                                        "rcppay_de": r["rcppay_de"],
                ...
                result.append(
                    {
                        "expect_de": datetime.strptime(
                            params["expect_de"], "%Y-%m-%d"
                        ).strftime("%Y-%m"),
                        "data": total_result,
                        "pData": total_rows,
                    }
        ...
        for key in total_long_result:
            ...
            sorted_finals = []
            for cntrct_sn in sorted(
                list(finals_data.keys()), key=lambda x: finals_data[x][0]["dlivy_de"]
            ):
        ...
        for key in total_long_result:
            if key == -1:
                continue
            results = total_long_result[key]
            func = lambda x: x["expect_de"] != "0000-00" and datetime.strptime(
                datetime.now(timezone("Asia/Seoul")).strftime("%Y-%m-01"), "%Y-%m-%d"
            ) > datetime.strptime(x["expect_de"], "%Y-%m")
        ...
        for cntrct_sn in sorted(
            ...
            sorted_finals += finals_data[cntrct_sn]
        sorted_finals = [
            _ for _ in sorted_finals if int(_["cntrct_sn"]) not in (1931, 517)
        ]
...
def equip_to_account():
    ...
    if "before_dlnt" not in params:
        params["before_dlnt"] = str(
            int(equipment["dlnt"]) - int(equipment["before_dlnt"])
        )
    ...
    row["model_no"] = (
        equipment["model_no"]
        if "rcppay_dtls" not in params
        else "{}{}".format(equipment["bigo"], params["rcppay_dtls"])
    )
    ...
    g.curs.execute(
        "UPDATE equipment SET dlivy_de=%(dlivy_de)s, before_dlnt=%(last_dlnt)s WHERE eq_sn=%(eq_sn)s",
        params,
    )
    return jsonify({"status": True, "message": "성공적으로 추가되었습니다."})
...
def insert_BF_ms_equip():
    try:
        ...
        if params["msh_approval_type"] in ("S", "B"):
            ...
            data = []
            for delng_ty_code, stock_sn, model_no, dlnt, prduct_ty_code in zip(
                params["delng_ty_code[]"],
                params["stock_sn[]"],
                params["model_no[]"],
                params["qy[]"],
                params["prduct_ty_code[]"],
            ):
                if (
                    stock_sn == ""
                    and dlnt == ""
                    ...
                    continue
                data.append((delng_ty_code, stock_sn, model_no, dlnt, prduct_ty_code))

            stock_sns = [
                (stock_sn, delng_sn, model_no, dlnt, prduct_ty_code)
                for (
                    delng_ty_code,
                    stock_sn,
                    model_no,
                    dlnt,
                    prduct_ty_code,
                ), delng_sn in zip(data, delng_sns)
                if dlnt != "" and model_no != ""
            ]
            ...
            for delng_sn, dlnt, invn_sttus_code, return_de in zip(
                params["delng_sn[]"],
                params["return_dlnt[]"],
                params["stock_place[]"],
                params["return_de[]"],
                ...
                if dlnt != "" and invn_sttus_code != "" and return_de != "":
                    ...
                    for s in stocks[: min(len(stocks), int(dlnt))]:
                        st.insert_log(
                            s["stock_sn"], 1, invn_sttus_code, None, return_de
                        )
...
def insert_BD_ms_equip():
    ...
    if params["msh_approval_type"] in ("S", "B"):
        ...
        stock_sns = [
            (stock_sn, delng_sn, model_no, dlnt, prduct_ty_code)
            for delng_ty_code, stock_sn, delng_sn, model_no, dlnt, prduct_ty_code in zip(
                params["delng_ty_code[]"],
                params["stock_sn[]"],
                delng_sns,
                params["model_no[]"],
...
def insert_general_sales_NR_append():
    try:
        params = request.get_json()
        sales.insert_general_sales_NR_append(params)
        return jsonify({"status": True, "message": "성공적으로 처리되었습니다."})
    except Exception as e:
        print(e)
        return make_response(str(e), 500)
